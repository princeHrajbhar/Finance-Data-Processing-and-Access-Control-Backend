...
import { otpTemplate } from "../../bullMQ/utils/emailTemplate.js";
...
export const forgotPassword = async (email: string) => {
  ...
  const SAFE_RESPONSE = { message: "If that email is registered, an OTP has been sent." };

  const user = await User.findOne({ email });
  if (!user) return SAFE_RESPONSE;

  try {
    await assertOtpCooldown(email, "FORGOT_PASSWORD");
  } catch {
    // Swallow cooldown error on forgot-password to preserve enumeration protection.
    // The real throttle is the IP-level rate limiter on the route.
  ...
  await Promise.all([
    OTP.create({ email, otp: hashedOtp, type: "FORGOT_PASSWORD", expiresAt }),
    addEmailJob({ to: email, subject: "Reset your password", html: otpTemplate(otp) }),
  ]);
...
export const resetPassword = async (payload: {
  ...
  const otpDoc = await OTP.findOne({ email, type: "FORGOT_PASSWORD" }).select("+otp");
  if (!otpDoc) {
    throw new AppError("OTP not found or expired", 400, ErrorCode.OTP_NOT_FOUND);
  }
  ...
  // Delete OTP and ALL sessions (force re-login everywhere)
  await Promise.all([
    OTP.deleteOne({ _id: otpDoc._id }),
    Session.deleteMany({ userId: user._id }),
  ]);

  logger.info({ email }, "Password reset — all sessions invalidated");
